...
func TestEvaluateAllSkipsMarkedPureGroups(t *testing.T) {
	root := &cobra.Command{Use: "test"}
	group := &cobra.Command{
		Use:  "skill",
	...
	cmdmeta.Set(list, cmdmeta.Metadata{
		Domain:     "skill",
	...
	decisions := New([]*platform.Rule{{MaxRisk: platform.RiskRead}}).EvaluateAll(root)

	if _, ok := decisions["skill"]; ok {
		t.Fatalf("pure group was evaluated: %#v", decisions)
	}
	if decision, ok := decisions["skill/list"]; !ok || !decision.Allowed {
		t.Fatalf("leaf decision = %#v, found = %v", decision, ok)
	}
}
...
func TestApplyReturnsStructuredCommandDenial(t *testing.T) {
	...
	if !ok ||
		...
		problem.Subtype != errs.SubtypeCommandDenied {
		t.Fatalf("error = %#v, want typed policy.command_denied", err)
	}
...
func TestApplyAggregatesParentWhenAllChildrenDenied(t *testing.T) {
	...
	if cmdmeta.IsPureGroup(skill) {
		t.Fatal("aggregated denial retained the pure-group marker")
	}
