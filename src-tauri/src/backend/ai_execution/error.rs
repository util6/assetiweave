...
pub(crate) enum AiExecutionError {
    ...
    MemoryGenerationToolsUnavailable,
...
impl AiExecutionError {
    pub(crate) fn to_view(&self) -> AiExecutionErrorView {
        let (code, message, retryable) = match self {
            ...
            Self::MemoryGenerationToolsUnavailable => (
                "memory_generation_tools_unavailable",
                "Memory Generation tools require an ACP agent.",
                false,
            ),
            Self::AgentNotFound { .. } => (
                "agent_not_found",
            ...
            Self::ToolUseDenied => (
                "tool_use_denied",
                "The AI agent attempted a tool that is not allowed for this execution.",
                false,
            ),
            Self::Protocol { .. } => (
                "protocol_failed",
