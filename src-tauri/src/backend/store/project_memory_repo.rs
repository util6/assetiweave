...
pub(crate) async fn load_project_memory_latest_version_sqlx(
    ...
    let row = sqlx::query(
        "SELECT tenant_id, id, project_id, version_number, status, input_fingerprint, source_watermark, content_markdown, raw_output_json, error_message, created_at, updated_at FROM project_memory_versions WHERE tenant_id = ?1 AND project_id = ?2 AND status = 'succeeded' ORDER BY version_number DESC LIMIT 1",
    )
    .bind(tenant_id)
    .bind(project_id)
    .fetch_optional(pool)
