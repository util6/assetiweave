...
async fn count_memory_usage_events_sqlx(
    pool: &SqlitePool,
    tenant_id: &str,
    memory_kind: &str,
    memory_id: &str,
...
async fn list_memory_usage_events_sqlx(
    pool: &SqlitePool,
    tenant_id: &str,
    memory_kind: &str,
    memory_id: &str,
...
async fn test_memory_usage_events_record_count_and_list() {
    ...
    let count_b = count_memory_usage_events_sqlx(pool, "tenant-b", "recall_session", "session-1")
        .await
