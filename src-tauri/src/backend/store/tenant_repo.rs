...
pub(crate) async fn create_local_tenant_sqlx(
    ...
    if load_tenant_sqlx(pool, &slug).await?.is_some() {
        return Err(AppError::Conflict(format!("tenant already exists: {slug}")));
    }
    load_principal_sqlx(pool, principal_id)
        .await?
        .ok_or_else(|| AppError::NotFound(format!("principal not found: {principal_id}")))?;
    ...
    load_tenant_sqlx(pool, &slug)
        .await?
        .ok_or_else(|| AppError::NotFound(format!("tenant not found after create: {slug}")))
}
...
pub(crate) async fn load_local_request_context_sqlx(
    ...
    let principal = load_principal_sqlx(pool, LOCAL_PRINCIPAL_ID)
        .await?
        .ok_or_else(|| AppError::NotFound("local principal not found".to_string()))?;
    ...
    let tenant = load_tenant_sqlx(pool, &active_tenant_id)
        .await?
        .ok_or_else(|| {
            AppError::NotFound(format!("active tenant not found: {active_tenant_id}"))
        })?;
...
fn normalize_tenant_slug(value: &str) -> AppResult<String> {
    ...
    if slug.is_empty() {
        return Err(AppError::Validation("tenant slug is required".to_string()));
    }
    Ok(slug)
}
