...
pub(crate) fn scaffold_external_adapter(
    ...
    let target_dir = path_utils::expand_path(&params.directory)?;
...
fn validate_external_adapter_manifest(
    ...
    let path = path_utils::expand_path(manifest_path)?;
    if !path.is_file() {
...
fn read_codex_sessions(
    ...
    let root = path_utils::expand_path(&source.location)?;
    ...
    for row in rows {
        ...
        let path =
            path_utils::expand_path(&rollout_path).unwrap_or_else(|_| PathBuf::from(&rollout_path));
        if !path.is_file() {
            continue;
        }
...
fn read_opencode_sessions(
    ...
    let db_path = path_utils::expand_path(&source.location)?;
    if !db_path.is_file() {
        return Ok(Vec::new());
    }
    let conn = Connection::open(&db_path).map_err(|error| error.to_string())?;
