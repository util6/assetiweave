...
代码、测试、构建配置和 CLI `--help` 是当前实现的最终事实来源。GitHub Issue 及其 Agent Brief 是待办工作的执行契约；文档只保存用户需要的说明、已提炼的知识与必要的决策理由，不能形成与实现竞争的总册。
...
- `agent-docs/generated/surface-matrix.md` 与 `src-tauri/src/adapters/engine/surface_mapping.rs` 是由 `pnpm gen:surface-matrix` 生成的接口覆盖物，不应手工编辑。
...
| `docs/` | 正式文档 | 面向用户的指南与经提炼的长期知识 | Agent 工作材料、生成参考、任务计划 |
| `agent-docs/` | Agent 工作材料 | 治理规则、ADR、Feature Plan、调研、归档和生成参考 | 用户指南、与代码竞争的全局现状描述 |
| `CONTEXT.md` | 领域词汇表 | 已解决的项目特有术语 | 实现细节、规格、任务清单 |
...
| 类型 | 位置 | 唯一职责 |
| --- | --- | --- |
| 已实现行为 | 代码、测试、构建配置、CLI `--help` | 当前事实来源 |
| 待办工作 | GitHub Issue 与 Agent Brief | 工作契约 |
| 领域语言 | `CONTEXT.md` | 项目特有术语的简明词汇表 |
| 架构决策 | `agent-docs/adr/` | 难逆转、非显而易见且有真实取舍的决定 |
| 用户文档与长期知识 | `docs/` | 用户指南与从工作中提炼的稳定知识 |
| 工作材料 | `agent-docs/feature-plans/`、`work/`、`archive/` | 方案、调研、执行轨迹和必要追溯 |
| 生成参考 | `agent-docs/generated/` | 由单一命令重建的参考产物 |

规则：

1. 新文档按需创建；同一含义只能有一个权威位置。
2. Feature Plan、调研和审计不构成当前架构或产品状态；完成后只提炼仍有价值的知识或 ADR，其余归档或删除。
3. ADR 编号按“决策日期 → 记录日期 → Git 证据”排序。每份 ADR 都标明决策日期、记录日期和对应提交。
4. `docs/` 不保存全局需求、设计或任务总册；这类状态分别由 Issue、实现和 Git 历史表达。
5. 文档引用失效时，更新引用或删除过时材料，不保留空壳兼容路径。
...
5. **是否是说明或设计？**
   - 用户说明与稳定知识放 `docs/`；
   - Agent 方案、审计、ADR 与生成参考放 `agent-docs/`；
   - 领域词汇只放根目录 `CONTEXT.md`；
   - 不提交工具会话历史。
...
- [ ] 旧入口、旧样式和旧测试已随迁移删除。
- [ ] 生成契约已更新。
- [ ] 没有把已实现行为、待办契约、领域术语、ADR 与工作材料写成重复的事实来源。
- [ ] 已运行受影响范围的类型检查、测试和构建。
...
2. `AGENTS.md`、`CONTEXT.md` 与相关 ADR 已指向当前 `adapters` + `backend` 结构；后续代码移动必须同步更新受影响的知识或决策文档。
