# Memory 增量规格：时间窗口、三层漏斗与用户可编辑生成 Skill

- 日期：2026-09-14
- 状态：待用户评审
- 归属：AssetIWeave Memory 重写，补充 Issue #20 与 Issue #30
- 性质：产品与执行契约；不包含本轮生产实现
- 事实源：用户确认的产品决策、当前代码、`01-contract.md`、`08-bounded-evidence-execution-spec.md`
...
- 日期视图、项目筛选和 Markdown 大纲是同一结构化数据集的不同确定性投影；前端按 SQLite 中的时间与 project key 分组，不解析 Agent Markdown 来识别项目。
...
5. 将 M01–M22 映射到 `04-verification-matrix.md` 的自动化与桌面验证门禁。
