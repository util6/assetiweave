...
2. 输出:`agent-docs/generated/surface-matrix.md`,表列 = canonical 能力 × {Engine method(s)(含 alias)、CLI verb、Tauri command、risk、confirmation}。
...
- `agent-docs/generated/surface-matrix.md` 提交并在 `docs/knowledge/repository-structure.md` 登记为生成物(禁手改)。
- 豁免清单基线数记录进守卫,只减不增。
