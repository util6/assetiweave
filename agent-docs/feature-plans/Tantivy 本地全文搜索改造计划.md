...
- Phase 1：基线与基础架构 (Baseline & Foundation)
  - 引入 Tantivy 依赖并完成最小 Tokenizer 冒烟测试。
  - 明确“SQLite 单一事实来源 + Tantivy 派生缓存”架构。
  - 建立搜索 DTO、Schema 版本、索引路径、Reader 注册表及索引状态模型。
  - 验收：能在临时目录创建索引，中文/英文样例可被正确检索命中。

- Phase 2：会话搜索垂直切片 (Conversation Vertical Slice)
  - 为 Session 与内容卡片生成文档，覆盖 `session` 与 `web` 两套数据表。
  - 替换 `search_conversation_records` 内部实现，保留 SQL 回退。
  - 替换 Session 搜索路径，避免当前 `instr(lower(...)) + EXISTS` 扫描。
  - 卡片类型过滤保持多选，并从 Tantivy Facet 返回各类型数量。
  - 验收：原 CLI/前端测试不改期望仍通过，新增分词、模糊、同义词、权重排序测试。

- Phase 3：后台索引构建与新鲜度管理 (Background Index & Freshness)
  - 新增搜索索引后台任务 Registry，支持 rebuild/status/progress。
  - 会话同步/导入、合并/拆分、翻译、删除等变更后按 session/question 增量更新索引。
  - Tauri 命令继续采用 `async + spawn_blocking`，不持有 `AppState.lock` 执行查询或重建。
  - UI 在搜索框和结果区展示加载/索引构建状态，不出现系统转圈式卡顿。

- Phase 4：推广至其他本地搜索表面 (Other Local Search Surfaces)
  - Catalog 资产、来源、Skill 分组接入 `search.local`，结果再从 SQLite hydration，前端不再对大数组做主搜索。
  - Prompt notes 先迁入 Engine/SQLite，再接入 Tantivy；迁移前仍只保留防抖的客户端过滤。
  - 手册页和 GitHub Skill discovery 不纳入 Tantivy：前者数据量小，后者是远程 provider 搜索。

- Phase 5：稳定性加固与容灾 (Hardening)
  - 对过期/缺失/损坏的索引自动回退，并提示用户重建。
  - 限制 `limit <= 500`、分页 offset、fuzzy 复杂度，防止单次查询拖垮 UI。
  - 高亮只返回纯文本片段和位置，前端用 React 渲染，不拼接原生 HTML。
