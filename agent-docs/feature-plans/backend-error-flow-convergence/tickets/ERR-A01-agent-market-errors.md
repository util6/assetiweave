...
- [ ] 列出目标模块全部跨模块 `Result<T,String>`，按 repository、invalid catalog、artifact/distribution、runtime/protocol 分类。
- [ ] 增加同一非法 Catalog、缺失 installation、SQL failure、artifact mismatch 和 process timeout 通过 AppService、Tauri、Engine 的 parity tests。
- [ ] 断言每场景的 code、retryable、安全 message/details 和内部 source；运行并记录当前字符串错误的 RED。
- [ ] 使用 `thiserror` 建立 Agent Market typed errors；repository 保留 SQLx source，Catalog validation 保留结构字段，process 保留 HostProcessError。
- [ ] AppService 使用显式 `From` 转成 AppError；adapter 只消费 WireError，不解析 Display 文本。
- [ ] 目标模块跨模块公开 `Result<T,String>` 归零；立即消费的私有 parser 可保留，但必须在交接列出。
