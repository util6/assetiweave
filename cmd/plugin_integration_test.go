...
func TestFailClosedPluginInstallBlocksCommandExecution(t *testing.T) {
	...
	assertPluginError(t, err, "plugin_install")
}
...
func TestStartupFailureBlocksCommandExecution(t *testing.T) {
	...
	assertPluginError(t, err, "plugin_lifecycle")
}
...
func assertPluginError(t *testing.T, err error, wantType string) {
	t.Helper()
	var exitErr *output.ExitError
	if !errors.As(err, &exitErr) || exitErr.Detail.Type != wantType {
		t.Fatalf("error = %#v, want %s ExitError", err, wantType)
	}
}
