# Implementation Plan：Agent Market 与动态 Runtime
...
**Acceptance**：所有冻结项有明确结论；偏离项已同步所有分册。
**Verify**：文档评审。
**依赖**：None。
...
**Tests first**：记录九条 builtin definition；OpenCode ACP fail 当前 compatibility 分支；不同 DB path singleton 现状。
**Acceptance**：只加 characterization，不改变生产行为；测试名明确标注将被 T12/T14/T26 替换。
**Verify**：
...
**Tests first**：CAT-05..08、readiness truth table、error serialization/redaction。
**Acceptance**：
...
**Tests first**：tenant isolation、CHECK constraints、upsert/list/candidate/health/delete、transaction rollback。
**Acceptance**：DDL 与 `06` 一致；一个 tenant/agent 一条 current row；无 catalog/history/hash 表。
**Verify**：
...
**Tests first**：CAT-01、CAT-05..09；九 Agent 分发形态断言；Kiro/Qoder command 漂移回归。
**Acceptance**：固定版本、无 latest/range、唯一 ID、标准 ACP item 数据驱动；所有来源字段可追踪。
**Verify**：
...
**Tests first**：CAT-02..04、5 MiB limit、invalid cache、atomic replace failure、ETag 304。
**Acceptance**：fetcher/clock/filesystem 可注入；失败不覆盖旧 catalog；无用户自定义 URL。
**Verify**：
...
**Tests first**：DST-01..03、07、10；显式不可用 choice 不 fallback；preview token 稳定/变更。
**Acceptance**：默认 System > Binary > Npx > Uvx；选择算法无网络/文件写；request 不含 program/args/env。
**Verify**：
...
**Tests first**：compatible/incompatible/missing version probe；argv no-shell；System uninstall ownership metadata。
**Acceptance**：installer 不写 DB/Registry；System `install_dir=None`；不 hash/copy/chmod/delete executable。
**Verify**：
...
**Tests first**：DST-04..06；wrong target；cancel；普通 fixture success；redirect/size limits。
**Acceptance**：解压前 hash；拒绝 traversal/links/special files/duplicates；全部输出位于 staging；无真实网络测试。
**Verify**：
...
**Tests first**：DST-07..09；fake npm argv；scripts denied；lock mismatch；local bin boundary；cancel/timeout。
**Acceptance**：exact package/version；强制 ignore scripts；结果 program local；执行 definition 无 `npx/-y`。
**Verify**：
...
**Tests first**：DST-10..11；fake uv env/argv；exact spec；global tool dir 未触碰；cancel/timeout。
**Acceptance**：`UV_TOOL_DIR/BIN_DIR` 指向 staging；结果 program local；执行 definition 无 `uvx`。
**Verify**：
...
**Tests first**：initialize/session/new/close；无 prompt/MCP；permission denied；timeout/cancel/child cleanup；model optional。
**Acceptance**：不复制 ACP client；输出 typed health；所有 terminal path 无 process tree。
**Verify**：
...
**Tests first**：empty fresh Registry；one definition；duplicate/invalid definition fail；old snapshot survives failed publish；concurrent read sees old/new complete snapshot。
**Acceptance**：无 builtin definitions；Registry 不依赖 DB/catalog；lookup 纯内存；只接受 Runtime Manager 已筛选的 execution-ready definitions；generation 成功时递增。
**Verify**：
...
**Tests first**：active count per Agent；swap 后 running execution 仍用旧 definition；new execution 用新 definition；mutation gate race；lock/permit cleanup。
**Acceptance**：active map 包含 agent/installation；Runtime Manager 有 panic-safe per-agent mutation gate；新 execution 不越过 lifecycle 临界区；无 I/O while lock held；`cancel_all` 保留退出用途。
**Verify**：
...
**Tests first**：两个临时 DB 有独立 Registry；manager 从 repository 发布 ready definitions；failed publish keeps old；shim 不缓存跨 DB 状态。
**Acceptance**：无 `SHARED_AGENT_EXECUTION_RUNTIME` static/OnceLock；Manager 是 DB/Registry composition owner；依赖可注入；测试无需 reset global；deprecated shim 不持有全局状态并标注 T15A 删除。
**Verify**：
...
**Tests first**：AppState 持有 manager；各 workflow 注入 fake runtime；未 ready error；不同 service 不串 DB。
**Acceptance**：除明确留给 T15A 的 Memory Dream 调用外，调用者不再使用 deprecated shim；业务不自行构建 Registry。
**Verify**：
...
**Tests first**：Memory Dream 注入 manager/fake runtime；两个 DB 不串；未 ready 稳定错误。
**Acceptance**：`rg shared_agent_execution_runtime src-tauri/src` 无生产定义/引用；没有新的 global runtime constructor。
**Verify**：
...
**Tests first**：LIFE-01..06；managed conformance fail no row；System failed diagnostic row；activation compensation。
**Acceptance**：per-agent lease；无 global app lock；staging never Registry；old snapshot on failure；typed phase/error。
**Verify**：
...
**Tests first**：download/hash/conformance/DB/Registry failure preserves old；ownership switch；same-version reinstall；catalog fixed version missing。
**Acceptance**：manual only；无长期 history；成功后才清 old dir；cleanup failure warning 不回滚可用新版本。
**Verify**：
...
**Tests first**：agent_in_use；assignment preview/confirm；System no delete；unsafe path reject；enable/disable Registry remove/add。
**Acceptance**：不自动取消 execution；不自动选择替代 Agent；删除前 canonical ancestor proof。
**Verify**：
...
**Tests first**：24h staging；missing entry broken；orphan boundary；Registry load；no network/protocol fanout。
**Acceptance**：bounded/background capable；不删除不确定目录；启动可继续并输出结构化 warning。
**Verify**：
...
**Tests first**：Market merge；preview input ignores arbitrary execution fields；installed health；compat methods；structured errors。
**Acceptance**：业务逻辑不在 adapter；路径 display 为 `~`；兼容 API callable；no probe-all list。
**Verify**：
...
**Tests first**：same Agent dedupe；different Agent bounded tasks；terminal merge/retention；cancel；shutdown active count。
**Acceptance**：task key tenant/agent；snapshot 无 secret；Agent task 与 AI execution task 分离。
**Verify**：
...
**Tests first**：start quick snapshot；event phase；poll terminal；cancel；unrelated command while task running；close active report。
**Acceptance**：无 global lock across I/O；invoke handler 注册完整；业务委托 AppService/lifecycle。
**Verify**：
...
**Tests first**：method exposure、DTO schema、risk/confirmation、兼容 aliases；run 等待终态；context cancel 收敛。
**Acceptance**：不手改 generated JSON；Tauri/Engine 共用 lifecycle service；Engine 不暴露不可跨进程轮询的 task ID；contract diff 只含预期方法/DTO。
**Verify**：
...
**Tests first**：CLI-01..04；preview/yes/json；单次 run；Ctrl-C；failed/cancelled exit code；Engine method names。
**Acceptance**：CLI 无 npm/uv/fs/SQLite；不跨 Engine 进程轮询 task；`--json` stdout 纯 DTO；confirmation policy 正确。
**Verify**：
...
### Checkpoint E：Cross-surface API
...
**Tests first**：MIG-01、MIG-02、MIG-04..06；assigned/unassigned；System pass/fail；Npx no network；repeat；managed choice preserved。
**Acceptance**：SQL migration transaction 不启动进程；no silent install/fallback；assignment/model 保留；启动可观察 notice；Desktop/Engine 共享 workflow；幂等。
**Verify**：
...
**Tests first**：OC-01..10、MIG-03/07；compat connection method；no `opencode run` spawn。
**Acceptance**：`rg cli_fallback` 无生产字段/分支；ACP fail => connected false；Translation 无 CLI execution route。
**Verify**：
...
**Tests first**：invoke command/params/schema；error；path/display；compat wrapper。
**Acceptance**：所有 invoke 在 service；组件无 backend DTO 猜测；uninstalled 无临时 command。
**Verify**：
...
**Tests first**：API-03；terminal wins；poll fallback；same Agent active lookup；cancel；listener cleanup。
**Acceptance**：与 AiExecution Provider 分离；无 page-level 全局 busy；Provider error 不清空已知 task。
**Verify**：
...
**Tests first**：UI-01..03；offline catalog；System/Binary choice；preview before start；task progress；current platform incompatible。
**Acceptance**：dense workspace style；protocol/distribution分开；不 probe-all；无 Add Custom；只禁用冲突 Agent。
**Verify**：
...
**Tests first**：UI-03/05；System no-delete copy；assignment conflict；agent_in_use；degraded health；manual update。
**Acceptance**：installed != connected；ownership/status visible；动作调用 preview/service；无静默 replacement。
**Verify**：
...
**Tests first**：UI-01/04；only ready candidates；current unavailable + CTA；model lazy；no Promise.all probe-all；no command/package map。
**Acceptance**：前端 Agent ID/command/package source of truth 消失；保存错误可恢复；icon metadata 不控制行为。
**Verify**：
...
**Tests first**：CAT-09；no runtime package manager；Gemini legacy 不回归。
**Acceptance**：
...
- `specs/feature-plans/agent-marketplace-dynamic-runtime/10-progress.md`
- `specs/feature-plans/SPEC_ Agent Marketplace 与动态运行时.md`
- `specs/design.md`（架构已成为事实后）
- `specs/requirements.md`（产品范围已成为事实后）
- `specs/tasks.md`（Git/测试证据证明后）
...
**Acceptance**：Release Acceptance 逐项有 PASS/证据；未通过项保持 Pending，不把目标写成现状。
**Verify**：
