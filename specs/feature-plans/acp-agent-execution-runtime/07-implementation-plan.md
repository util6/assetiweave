# Implementation Plan: ACP Agent Execution Runtime
...
**Status**：Complete（2026-08-13；采用全部推荐默认值，见 `09-progress.md`）
...
**Status**：Complete（2026-08-13；`cargo check --manifest-path src-tauri/Cargo.toml` PASS）
...
**Tests first**：REG-03、REG-04、REG-05。
...
**Tests first**：REG-01、REG-02、REG-06、REG-07。
...
**Tests first**：EXE-01、model/prompt validation、error redaction。
...
**Tests first**：现有 timeout/process group tests 保持；新增 helper 幂等/平台测试。
...
**Tests first**：PROC-01..06、PROC-10、PROC-13、PROC-14。
...
**Tests first**：PROC-07..09、PROC-15。
...
**Tests first**：ACP-01..04、ACP-10、ACP-17、ACP-18。
...
**Tests first**：ACP-05..09、ACP-11..16、ACP-19。
...
**Tests first**：Fake modes + LIFE-01..11。
...
**Tests first**：injected fake runtime，AppService constructors。
...
**Tests first**：TR-01..06、TR-09、TR-10、TR-12。
...
**Tests first**：REG-08..10、TR-07、TR-08。
...
**Verify**：card translation + agents tests。
...
**Verify**：Tauri adapter tests + cargo check。
...
**Tests first**：service/provider cases。
...
**Tests first**：start/progress/unrelated controls/cancel/success/save failure/unmount。
...
**Verify**：component test + typecheck。
...
**Tests first**：TAURI-07/08、global indicator cases。
...
**Verify**：frontend tests + Rust close tests。
