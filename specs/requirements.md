...
- 2026-05-30：视图语义分流与源级批量挂载里程碑。当前代码状态确认资产总览目录为“列表/卡片”切换，技能源管理为“列表/分栏”切换；Toolbar 组件统一但视图选项按页面传入；源级 Skill 批量挂载写入同一份 `asset_mounts` 关系。已通过 `pnpm typecheck`、`pnpm test`、`pnpm build` 和 in-app browser 手工验收。构建仍有 Vite 单 chunk 超过 500 kB 的体积提示，暂不影响功能通过。
