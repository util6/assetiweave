...
16. 前端目录结构已明确为 `app/components/config/hooks/i18n/layouts/mock/pages/router/schemas/services/store/styles/types/utils`，其中 `services` 和 `pages` 保持当前项目约定。
17. App 快捷入口支持真实应用图标 token 和自定义 SVG path 资源，并可在设置页编辑。
18. 导航菜单支持中英文本地化 label 覆盖，设置页编辑当前语言对应文案。
19. Tauri 后端支持按资产 kind 查询/扫描、取消真实挂载并返回最新挂载状态、通过 dialog plugin 选择导入目录。
20. 当前自动化验证基线包含 `pnpm typecheck`、`pnpm test`、`cargo test`、`pnpm build`。
