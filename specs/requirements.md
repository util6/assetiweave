...
AssetIWeave 是一个本地优先的 AI 文件资产管理桌面应用。它管理的不是某一种固定格式，而是 AI 工具共同演进出的文件资产形态，包括 prompt、rules、memory、skills、MCP 配置、agent 定义、slash command、workflow 等。

核心目标是把分散在不同目录、仓库和工具配置中的 AI 文件资产集中编目、分类、标记、分组，并按不同 CLI/App 的能力画像部署出去。目标目录只是由 AssetIWeave 生成的投影结果，不是唯一真实来源。
...
- MVP 不实现远程账号、云同步、多设备协同。
- MVP 不编辑源资产正文，只管理元数据和部署策略。
- MVP 不直接修改第三方工具数据库。
- MVP 不依赖现有 Python 同步脚本或 launchd 任务。
- MVP 不做完整 CLI；后续可以补充轻量 CLI 作为自动化入口。
...
- 决策优先级清晰：显式禁用 > 显式排除 > 显式包含 > Profile 规则 > 默认策略。
...
作为用户，我希望 AssetIWeave 能把资产部署到各个 AI 工具目标目录，这样工具可以直接使用这些资产。

验收标准：

- MVP 支持 `symlink` 和 `copy` 两种部署策略。
...
- MVP 可以只读扫描 `~/.cc-switch/skills` 作为普通本地源。
...
4. 用元数据覆盖层管理标签、分组和启用状态。
5. 管理目标 Profile。
6. 生成部署计划。
7. 以 symlink/copy 执行部署。
8. 展示冲突和解释原因。
