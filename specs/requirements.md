...
当前架构决策：桌面 App、Go CLI 和后续自动化入口必须共享同一套 Rust Engine/AppService 后端。前端和 CLI 不能直接写 SQLite，也不能各自实现扫描、挂载、计划或执行规则。
...
- v1 内部只保留外部适配器执行器，以及 Codex、OpenCode 的内置兜底适配器。
- Claude Code、ZCode 等其它来源通过显式注册可信外部适配器脚本接入；脚本输出标准化 Session/Turn/Part，AssetIWeave 负责分组、搜索、存储和导出。
...
- Conversation 是本地历史归档；来源 Session 被归档、删除或暂时不可见时，已导入内容继续可浏览、搜索和导出。
- 默认同步完整发现轻量 Session 元数据，只读取新建或来源版本变化的 active Session；任意年龄的旧 Session 重新产生内容后必须被识别并更新。
- Conversation 页面同步按钮保持增量模式；全局设置提供需明确确认的全量同步入口，用当前解析器重新读取所有仍可发现的 Session 与 Web Record。
- 全量同步只强制重新解析当前可发现内容，不把来源中已归档、删除或暂时不可见的历史推导为删除；这些本地历史继续保留。
- 全量同步作为后台任务运行；全局设置实时展示准备、当前来源、已完成来源数、总来源数和百分比，重新进入设置后可从任务快照恢复进度。
...
13. 资产总览目录支持列表视图和卡片视图切换。
14. 技能源管理页支持列表视图和分栏视图切换。
15. 技能源管理页支持按来源批量挂载该来源下所有 Skill 到指定 App/Profile。
16. 技能源导入入口和目录选择 UI 已接入。
17. Skill Group 管理已实现 CRUD、规则匹配、手动成员、批量挂载和独占挂载。
18. 技能源管理页已接入互联网 Skill 搜索、预览和导入入口。
19. CLI 已具备 `skill search` / `skill acquire` / `skill remote check`，可从 GitHub 候选中定位 `SKILL.md`、导入备份库并检查远程 drift。
20. CLI 已形成 Engine contract、命令策略、插件平台、harvester/webharvester、自更新和稳定错误分类等基础设施。
21. 前端目录结构已明确为 `app/components/config/hooks/i18n/layouts/mock/pages/router/schemas/services/store/styles/theme/types/utils`，其中 `services` 和 `pages` 保持当前项目约定。
22. App 快捷入口支持真实应用图标 token 和自定义 SVG path 资源，并可在设置页编辑。
23. 导航菜单支持中英文本地化 label 覆盖，设置页编辑当前语言对应文案。
24. Tauri 后端支持按资产 kind 查询/扫描、取消真实挂载并返回最新挂载状态、通过 dialog plugin 选择导入目录。
25. 启动和关闭路径支持已记录资产/挂载观测刷新、后台任务退出提示、关闭前数据库备份。
26. 当前自动化验证基线包含 `pnpm typecheck`、`pnpm test`、`cargo test`、`pnpm build`；CLI 变更还需要 `go vet -C cli ./...`、`go test -C cli -race ./...`、`pnpm cli:contract` 或 `pnpm cli:test:e2e` 视范围执行。
...
29. Conversation Adapter CLI 在 `conversation adapter` 子树暴露 `list`、`inspect` 与 `upgrade`：默认扫描 `~/.assetiweave/conversation-adapters/<cli-name>`，`-d` 使用代码仓库 `builtin-assets/adapters`，也可传入任意单个 Adapter 目录。升级必须通过共享 Rust Engine/AppService 生成快照、probe，并仅在成功后激活 `packages` 中的不可变运行副本。
...
- MCP 配置合并。
- 完整后台常驻同步。
- 通用资产在线仓库和 Remote Skill 市场化；Conversation Adapter Package 市场第一阶段已经实现。

### 3.1 技术约束

- App 必须是 Tauri 2 桌面应用。
- 前端使用 React 19、TypeScript、Vite 和 Tailwind CSS；可复用视觉规则应沉淀到 `frontend/src/theme/`、`components/ui`、`components/foundation` 和 `components/common`，不要在页面中扩展大段一次性样式。
- Rust 侧使用单一 `src-tauri` package。Tauri App 和 stdio Engine 共享 `assetiweave_lib`，业务入口通过 `backend/application/AppService`，接口适配放在 `adapters/tauri` 和 `adapters/engine`。
- Go CLI 使用 Cobra，通过 stdio Engine JSON protocol 调用 Rust 后端；CLI 不直接写 SQLite、不直接创建或删除挂载链接。
- 持久化使用 SQLite；schema 只能通过 `src-tauri/migrations/` 演进；Engine/CLI 契约通过 `pnpm cli:contract` 生成。
- 配置导出/导入、资产导出、备份和恢复是 app-owned 数据能力，不改变源目录只读原则。
- 任何扫描目录、复制文件、同步对话、远程导入、批量刷新或批量挂载的功能都必须按后台任务模型设计，避免阻塞全局 UI 或持有全局 app lock 执行 I/O。
