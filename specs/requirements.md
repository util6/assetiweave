...
27. Conversation Adapter Package 市场第一阶段已完成：区分 built-in、托管安装、本地目录、Git ref、legacy external 和 dev override；支持 preflight 后注册/注销/安装/更新/卸载、Catalog v2、版本历史、changelog、兼容更新检测和指定兼容版本安装。
28. 插件市场托管操作使用后台任务；外部目录和 Git 工作区永不由 AssetIWeave 删除，卸载只清理应用托管 package library，并保留 Conversation records 与 Source 配置。
...
- Conversation Adapter 支持已安装版本直接切换、一键回退、单独删除非 active 托管版本，以及 `manual`、`follow_stable`、`follow_beta`、`pin_exact` 更新策略；跟随策略只改变更新候选和提示，不静默安装远端代码。
