...
export function AppUpdateDialog() {
  ...
  const retryText =
    state.retryAttempt && state.retryTotal
      ? t("update.retrying", {
          attempt: state.retryAttempt,
          total: state.retryTotal,
        })
      : "";
  ...
  return (
    <DialogFrame
      ...
      icon={
        introMode ? <CheckCircle2 size={20} /> : getDialogIcon(state.status)
      }
      iconClassName={cn(
        "border-status-update/30 bg-status-update/15 text-status-update",
        (introMode || state.status === "ready") &&
          "border-status-create/30 bg-status-create/15 text-status-create",
        !introMode &&
          state.status === "error" &&
          "border-status-remove/30 bg-status-remove/15 text-status-remove",
      )}
      onClose={handleClose}
      size="md"
        ...
        <div className="space-y-3">
          <div className="rounded-lg border border-theme-card-border bg-theme-card/75 p-3">
            <p className="text-body-md font-semibold text-on-surface">
              {getStatusTitle(state.status, t)}
            </p>
            <p className="mt-1 text-body-sm text-on-surface-variant">
              {getStatusDescription(state.status, t)}
            </p>
            {retryText && (
          ...
          {state.info && (
            <div className="grid gap-2 rounded-lg border border-theme-card-border bg-theme-panel/70 p-3 text-body-sm">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="font-semibold text-status-update">
              ...
              {state.info.date && (
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="font-medium text-on-surface">
                    {formatDate(state.info.date, locale)}
                  </span>
                </div>
              )}
            </div>
          ...
          {(state.status === "downloading" ||
            ...
            <div className="space-y-2">
              <div className="h-2 overflow-hidden rounded-full bg-theme-control">
                <div
                  className="h-full rounded-full bg-status-update transition-[width]"
                  style={{
...
function CurrentVersionIntro({ currentVersion }: { currentVersion?: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-theme-card-border bg-theme-card/75 p-3">
        <p className="text-body-md font-semibold text-on-surface">
          {t("update.intro.currentTitle")}
        </p>
        <p className="mt-1 text-body-sm text-on-surface-variant">
          {t("update.intro.currentDescription")}
        </p>
      </div>
      ...
      <div className="grid gap-2 rounded-lg border border-theme-card-border bg-theme-panel/70 p-3 text-body-sm">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="font-semibold text-status-create">
            {currentVersion ? `v${currentVersion}` : t("common.none")}
          </span>
        </div>
      </div>
