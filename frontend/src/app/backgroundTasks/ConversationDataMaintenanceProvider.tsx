...
const ConversationDataMaintenanceContext =
  createContext<ConversationDataMaintenanceContextValue | null>(null);
...
export function ConversationDataMaintenanceProvider({
  ...
  >(
    () => ({
      ...
      subscribe: (listener) =>
        subscribeConversationDataMaintenanceTasks(listener),
    }),
    [],
  );
...
export function useConversationDataMaintenance() {
  ...
  if (!context) {
    throw new Error(
      "useConversationDataMaintenance must be used inside ConversationDataMaintenanceProvider",
    );
  }
  return context;
}
