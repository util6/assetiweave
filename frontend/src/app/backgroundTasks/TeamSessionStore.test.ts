...
describe("TeamSessionStore", () => {
  it("keeps member timelines isolated and ignores an older duplicate snapshot", () => {
    ...
    state = applyTeamMemberStreamSnapshot(
      state,
      streamSnapshot("leader", "execution-a", 1, [
        item("assistant-a", "assistant_text", "hello", 1),
      ]),
    );
    state = applyTeamMemberStreamSnapshot(
      state,
      streamSnapshot("teammate", "execution-b", 1, [
        item("assistant-b", "assistant_text", "teammate", 1),
      ]),
    );
    state = applyTeamMemberStreamSnapshot(
      state,
      streamSnapshot("leader", "execution-a", 2, [
    ...
    expect(
      selectTeamMemberSession(state, "team-1", "leader")?.stream.items,
    ).toHaveLength(1);
    expect(
      selectTeamMemberSession(state, "team-1", "leader")?.stream.items[0]?.text,
    ).toBe("hello world");
    expect(
      selectTeamMemberSession(state, "team-1", "teammate")?.stream.items[0]
        ?.text,
    ).toBe("teammate");
  });
  ...
  it("keeps the latest task status even when a task update reuses the stream sequence", () => {
    ...
    state = applyTeamMemberStreamSnapshot(
      state,
      streamSnapshot("leader", "execution-a", 3, [], false, "Running"),
    );
    state = applyTeamMemberStreamSnapshot(
      state,
      streamSnapshot("leader", "execution-a", 3, [], false, "Succeeded"),
    );

    expect(
      selectTeamMemberSession(state, "team-1", "leader")?.task?.state,
    ).toBe("Succeeded");
    expect(
      selectTeamMemberSession(state, "team-1", "leader")?.restore_state,
    ).toBe("ready");
  });
  ...
  it("exposes restoring, partial, and unavailable recovery states and bounds the timeline", () => {
    ...
    restoring = applyTeamMemberStreamSnapshot(
      restoring,
      streamSnapshot("leader", "replay-running", 1, [], true),
    );
    expect(
      selectTeamMemberSession(restoring, "team-1", "leader")?.restore_state,
    ).toBe("restoring");

    let partial = createTeamSessionStoreState("team-1");
    partial = applyTeamMemberStreamSnapshot(
      partial,
      streamSnapshot("leader", "replay-partial", 1, [], true, "Succeeded"),
    );
    expect(
      selectTeamMemberSession(partial, "team-1", "leader")?.restore_state,
    ).toBe("partial");
    ...
    expect(
      selectTeamMemberSession(unavailable, "team-1", "leader")?.restore_state,
    ).toBe("unavailable");

    const manyItems = Array.from({ length: 300 }, (_, index) =>
      item(`item-${index}`, "assistant_text", `${index}`, index + 1),
    );
    let bounded = createTeamSessionStoreState("team-1");
    bounded = applyTeamMemberStreamSnapshot(
      bounded,
      streamSnapshot("leader", "execution-many", 300, manyItems),
    );
    expect(
      selectTeamMemberSession(bounded, "team-1", "leader")?.stream.items,
    ).toHaveLength(256);
  });
});
