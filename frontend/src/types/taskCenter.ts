...
export function normalizeTaskView(raw: unknown): TaskView {
  ...
  const normalizedStages: TaskStageView[] = rawStages.map((stRaw) => {
    const st = (stRaw && typeof stRaw === "object" ? stRaw : {}) as Record<string, unknown>;
    const rawActivities = (st.currentActivities ?? st.current_activities ?? []) as unknown[];
    const normalizedActivities: TaskActivityView[] = rawActivities.map((actRaw) => {
      const act = (actRaw && typeof actRaw === "object" ? actRaw : {}) as Record<string, unknown>;
      ...
      const displayPath = (act.displayPath ?? act.display_path ?? null) as string | null;
    ...
    const durationMs = typeof st.durationMs === "number" ? st.durationMs : typeof st.duration_ms === "number" ? st.duration_ms : null;
    const startedAt = (st.startedAt ?? st.started_at ?? null) as string | null;
    const finishedAt = (st.finishedAt ?? st.finished_at ?? null) as string | null;
  ...
  const startedAt = (data.startedAt ?? data.started_at ?? null) as string | null;
  const updatedAt = (data.updatedAt ?? data.updated_at ?? null) as string | null;
  const finishedAt = (data.finishedAt ?? data.finished_at ?? null) as string | null;
  const errorSummary = (data.errorSummary ?? data.error_summary ?? null) as string | null;
  const resultSummary = (data.resultSummary ?? data.result_summary ?? null) as string | null;
  ...
  return {
    ...
    capabilities: (data.capabilities ?? { cancellable: false, retryable: false, clearable: false }) as TaskCapabilitiesView,
