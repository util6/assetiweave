...
describe("Memory public schemas", () => {
  it("parses a persistent structured Recall session", () => {
    expect(
      memoryRecallSessionSchema.parse(session).turns[0].structuredOutput
        ?.answer,
    ).toContain("shared service");
  });

  it("rejects unknown structured output fields", () => {
    expect(() =>
      memoryRecallSessionSchema.parse({
        ...session,
        turns: [
          {
            ...session.turns[0],
            structuredOutput: {
              ...session.turns[0].structuredOutput,
              hidden: true,
            },
          },
        ],
