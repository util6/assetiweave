...
describe("updaterRetry", () => {
  it("retries transient network and server failures", () => {
    expect(
      isRetryableUpdaterError(new Error("timeout while checking update")),
    ).toBe(true);
  ...
  it("does not retry permanent updater failures", () => {
    expect(isRetryableUpdaterError("signature verification failed")).toBe(
      false,
    );
  ...
  it("sanitizes URLs from error messages", () => {
    expect(
      sanitizeUpdaterError(
        "failed to fetch https://example.com/releases/latest.json token=secret",
      ),
    ).toBe("failed to fetch [URL] token=secret");
  });
});
