...
export function TeamPage() {
  ...
  const installedAgents = useMemo(() => {
    const installed = new Map(
      market
        .filter(
          (item) => item.installed?.enabled && item.installed.executionReady,
        )
  ...
  const agentCanFillTeam = (agentId: string) => {
    ...
    const capabilities =
      marketItem.installed?.capabilities ?? marketItem.capabilities;
    return Boolean(
      capabilities.resume &&
  ...
  const selectableAgents = () =>
    installedAgents.filter((agent) => agentCanFillTeam(agent.id));
  ...
  useEffect(() => {
    const leader =
      selectedTeam?.members.find((member) => member.role === "leader") ??
  ...
  useEffect(() => {
    ...
    void Promise.all([
      listTeams(),
      listAgentCatalog(),
      listAgentMarket({ installedOnly: true }),
    ])
      ...
      .catch((loadError: unknown) => {
        if (!cancelled)
          setError(t("team.error.load", { message: errorMessage(loadError) }));
      })
      .finally(() => {
  ...
  const loadModels = async (agentId: string) => {
    ...
    try {
      ...
      setModels((current) => ({
        ...current,
        [agentId]: result.available ? result.models : [],
      }));
    } catch (loadError: unknown) {
  ...
  const save = async (event: FormEvent) => {
    ...
    const leaderCount = formMembers.filter(
    ...
    if (
      ...
      formMembers.some(
        (member) =>
          !installedAgents.some((agent) => agent.id === member.agent_id),
      )
    ) {
    ...
    for (const member of formMembers) {
      ...
      const marketModel = market.find((item) => item.id === member.agent_id)
        ?.installed?.selectedModelId;
      if (
        member.model &&
        availableModels.length > 0 &&
        !availableModels.some((model) => model.id === member.model)
      ) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setTeams(nextTeams);
      setSelectedTeamId((current) =>
        current === deleting.id ? (nextTeams[0]?.id ?? null) : current,
      );
      setDeleting(null);
    ...
    try {
      setRunSnapshot(
        await draftTeam({
          ...
          leader_message: message.trim(),
        }),
      );
    } catch (draftError: unknown) {
      setWorkflowError(
        t("team.workflow.error", { message: errorMessage(draftError) }),
      );
    } finally {
  ...
  const saveRunReview = async () => {
    ...
    const orderedTasks = [...runSnapshot.tasks].sort(
      (left, right) => left.sort_order - right.sort_order,
    );
    const runTeammates = runSnapshot.run.roster_snapshot.filter(
      (member) => member.role === "teammate",
    );
    ...
    if (tasks.some((task) => !task.title || !task.description)) {
      setWorkflowError(
        t("team.workflow.error", {
          message: t("team.workflow.taskFieldsRequired"),
        }),
      );
      return;
    }
    if (
      tasks.some(
        (task) =>
          !runTeammates.some(
            (member) => member.member_id === task.owner_member_id,
          ),
      )
    ) {
      setWorkflowError(
        t("team.workflow.error", { message: t("team.workflow.ownerRequired") }),
      );
      return;
    }
    setWorkflowBusy(true);
    ...
    try {
      setRunSnapshot(
        await reviewTeamRun({
          ...
          revision: runSnapshot.run.revision,
          tasks,
        }),
      );
    } catch (reviewError: unknown) {
      setWorkflowError(
        t("team.workflow.error", { message: errorMessage(reviewError) }),
      );
    } finally {
      setWorkflowBusy(false);
    }
  ...
  const startTeamExecution = async () => {
    ...
    try {
      ...
      setWorkflowError(
        t("team.workflow.error", { message: errorMessage(confirmError) }),
      );
    } finally {
      setWorkflowBusy(false);
    }
  ...
  const cancelTeamExecution = async () => {
    ...
    try {
      ...
      setWorkflowError(
        t("team.workflow.error", { message: errorMessage(cancelError) }),
      );
    } finally {
      setWorkflowBusy(false);
    }
  ...
  const moveRunTask = (taskId: string, direction: -1 | 1) => {
    if (!runSnapshot) return;
    const orderedRunTasks = [...runSnapshot.tasks].sort(
      (left, right) => left.sort_order - right.sort_order,
    );
    const index = orderedRunTasks.findIndex((task) => task.id === taskId);
    const nextIndex = index + direction;
    if (index < 0 || nextIndex < 0 || nextIndex >= orderedRunTasks.length)
      return;
    ...
    >
      <section className="flex min-h-0 flex-1 flex-col gap-[var(--app-section-gap)] overflow-hidden px-[var(--app-page-x)] py-[var(--app-page-y)]">
        <PageHeader
          actions={
            <Button
              disabled={loading || installedAgents.length === 0}
              onClick={openCreate}
              size="sm"
              type="button"
            >
        ...
        {error && (
          <Panel
            className="border-status-remove/35 bg-status-remove/10 text-body-sm text-status-remove"
            padding="sm"
          >
            {error}
          </Panel>
        ...
        {loading ? (
          <EmptyState
            ...
            description={t("team.list.loading")}
            icon={<Users size={22} />}
            ...
            <div className="min-h-0 min-w-0">
              <div className="mb-2 hidden items-center justify-between gap-2 rounded-xl border border-theme-card-border/65 bg-theme-card/45 px-3 py-2 max-[860px]:flex">
                ...
                >
                  ...
                  {teamListOpen
                    ? t("team.chat.collapseNavigation")
                    : t("team.chat.expandNavigation")}
                </Button>
              </div>
              <nav
                aria-label={t("team.list.title")}
                className={`min-h-0 overflow-y-auto max-[860px]:max-h-52 ${teamListOpen ? "" : "max-[860px]:hidden"}`}
                id="team-roster-navigation"
              >
                <Panel className="min-h-full" padding="sm" variant="muted">
                  ...
                  {teams.length === 0 ? (
                    <EmptyState
                      ...
                      description={t("team.list.emptyDescription")}
                      icon={<Users size={20} />}
                      title={t("team.list.empty")}
                    />
                  ) : (
                      ...
                      {teams.map((team) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {leader && (
                              <div className="mt-2 flex items-center gap-1.5 text-caption text-primary">
                                <Shield size={13} />
                                {t("team.detail.leader")} · {leader.agent_id}
                              </div>
                            )}
                          </button>
                        );
            ...
            <div className="grid gap-3">
              <div>
                ...
                <p className="mt-2 text-caption text-on-surface-variant">
                  {t("team.detail.teamId")}: <code>{selectedTeam.id}</code>
                </p>
              </div>
              <div className="grid gap-2">
                ...
                <div className="divide-y divide-theme-card-border/60 overflow-hidden rounded-xl border border-theme-card-border/70">
                  {selectedTeam.members
                    ...
                    .map((member) => (
                      <div
                        className="flex flex-wrap items-center justify-between gap-3 bg-theme-card/35 px-4 py-3"
                        key={member.id}
                      >
                        <div className="flex min-w-0 items-center gap-3">
                          <span className="grid size-7 shrink-0 place-items-center rounded-lg bg-theme-control text-caption font-semibold text-on-surface-variant">
                            {member.sort_order + 1}
                          </span>
                        ...
                        <code className="text-caption text-on-surface-variant">
                          {member.execution_context_key.slice(0, 12)}…
                        </code>
                      </div>
                    ))}
                </div>
          ...
          <DialogFrame
            ...
            footer={
              <>
                ...
                <Button disabled={busy} form="team-editor-form" type="submit">
                  {busy ? t("common.saving") : t("team.action.save")}
                </Button>
              </>
            }
            icon={<Users size={18} />}
            ...
            title={
              editor === "new"
                ? t("team.dialog.createTitle")
                : t("team.dialog.editTitle")
            }
          >
            <form
            ...
            >
              <label className="grid gap-1.5 text-body-sm font-semibold">
                ...
                <Input
                  ...
                  placeholder={t("team.dialog.namePlaceholder")}
                  value={formName}
                />
              </label>
              <label className="grid gap-1.5 text-body-sm font-semibold">
                ...
                <Input
                  ...
                  placeholder={t("team.dialog.descriptionPlaceholder")}
              ...
              <div className="grid gap-2">
                <div className="flex items-center justify-between gap-3">
                  ...
                  >
                    <Plus size={14} />
                    {t("team.action.addMember")}
                  </Button>
                </div>
                <div className="grid max-h-72 gap-2 overflow-y-auto pr-1">
                  {formMembers.map((member, index) => {
                    const memberModels = models[member.agent_id] ?? [];
                    const roleAgents = selectableAgents();
                    return (
                      <div
                        className="grid gap-2 rounded-xl border border-theme-card-border/65 bg-theme-card/40 p-3"
                      ...
                      >
                        <div className="grid grid-cols-[auto_minmax(0,1fr)_minmax(0,1fr)_auto] items-end gap-2">
                          ...
                          <div className="flex gap-1">
                            ...
                            <Button
                              aria-label={t("team.action.moveDown")}
                              disabled={
                                busy || index === formMembers.length - 1
                              }
                              onClick={() => moveMember(index, 1)}
                              size="icon-sm"
                              type="button"
                            ...
                            <Button
                              aria-label={t("team.action.removeMember")}
                              disabled={busy || formMembers.length <= 2}
                              onClick={() =>
                                setFormMembers((current) =>
                                  current.filter(
              ...
              {formError && (
                <p className="rounded-xl border border-status-remove/35 bg-status-remove/10 p-3 text-body-sm text-status-remove">
                  {formError}
                </p>
              )}
            </form>
        ...
        <ConfirmDialog
          busy={busy}
          confirmLabel={t("team.action.delete")}
          message={
            deleting
              ? t("team.confirm.deleteMessage", { name: deleting.name })
              : ""
          }
