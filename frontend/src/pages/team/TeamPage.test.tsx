...
describe("TeamPage", () => {
  beforeEach(() => {
    ...
    listAgentModelsMock.mockImplementation(async (agentId: string) => ({
      agent_id: agentId,
      available: true,
      models: [
        {
          id: agentId === "agent-a" ? "model-a" : "model-b",
          label: agentId === "agent-a" ? "Model A" : "Model B",
          description: null,
        },
      ],
      current_model_id: agentId === "agent-a" ? "model-a" : "model-b",
      error_code: null,
      error: null,
    }));
    ...
    subscribeTeamMemberSessionsMock.mockImplementation(
      async (listener: (snapshot: TeamMemberStreamSnapshot) => void) => {
  ...
  it("filters members by the shared Resume, Replay, and Live Events facts", async () => {
    listAgentMarketMock.mockResolvedValue([
      {
        id: "agent-a",
        installed: { enabled: true, executionReady: true },
        capabilities: { resume: true, historyReplay: true, liveEvents: true },
      },
      {
        id: "agent-b",
        installed: { enabled: true, executionReady: true },
        capabilities: { resume: true, historyReplay: true, liveEvents: false },
      },
    ]);
  ...
  it("opens on the Leader session and switches one active timeline without changing member activity", async () => {
    const leader = memberStream(
      "leader",
      "execution-leader",
      "Running",
      "Leader timeline",
    );
    const teammate = memberStream(
      "teammate",
      "execution-teammate",
      "Running",
      "Teammate timeline",
    ...
    getTeamMemberStreamSnapshotMock.mockImplementation(
      async (_teamId: string, memberId: string) =>
        memberId === "leader" ? leader : teammate,
    );

    renderPage();
    await waitFor(() =>
      expect(screen.getByTestId("team-chat-shell")).toBeTruthy(),
    );

    expect(
      screen.getByTestId("team-member-leader").getAttribute("aria-selected"),
    ).toBe("true");
    ...
    await waitFor(() => {
      expect(
        screen
          .getByTestId("team-member-teammate")
    ...
    expect(
      screen.getByTestId("team-member-leader-status").textContent,
    ).toContain("Working");
  });
  ...
  it("collapses secondary navigation without hiding the active recipient or composer", async () => {
    ...
    expect(
      screen.getByRole("navigation", { name: "Teams" }).className,
    ).toContain("max-[860px]:hidden");
  ...
  it("sends to the active teammate and shows one optimistic user item before backend acceptance", async () => {
    let resolveStart: ((snapshot: TeamMemberStreamSnapshot) => void) | null =
      null;
    startTeamMemberTurnMock.mockImplementation(
      () =>
        new Promise<TeamMemberStreamSnapshot>((resolve) => {
    ...
    fireEvent.change(screen.getByLabelText("Message content"), {
      target: { value: "Inspect the cache" },
    });
    ...
    await act(async () => {
      resolveStart?.(
        streamSnapshotWithItems("teammate", "execution-direct", "Running", [
          sessionItem(
            "teammate",
            "execution-direct",
            "workflow:user",
    ...
    await waitFor(() =>
      expect(screen.getAllByText("Inspect the cache")).toHaveLength(1),
    );
  });

  it("uses the same composer for the default Leader recipient", async () => {
    startTeamMemberTurnMock.mockResolvedValue(
      streamSnapshotWithItems("leader", "execution-leader", "Running", [
        sessionItem(
          "leader",
          "execution-leader",
          "workflow:user",
  ...
  it("runs the Leader task mode inline and persists the edited plan before confirmation", async () => {
    ...
    expect(
      screen
        .getByRole("button", { name: "Team task" })
        .getAttribute("aria-pressed"),
    ).toBe("true");
    fireEvent.change(screen.getByLabelText("Message content"), {
      target: { value: "Split this work" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Generate draft" }));

    await waitFor(() =>
      expect(draftTeamMock).toHaveBeenCalledWith({
        team_id: "team-1",
        leader_message: "Split this work",
      }),
    );
    await waitFor(() =>
      expect(screen.getByTestId("team-plan-task-task-a")).toBeTruthy(),
    );
    fireEvent.change(screen.getByLabelText("Task title 1"), {
      target: { value: "Edited first" },
    });
    fireEvent.change(screen.getByLabelText("Task description 1"), {
      target: { value: "Edited first details" },
    });
    fireEvent.change(screen.getByLabelText("Owner 1"), {
    ...
    await waitFor(() =>
      expect(reviewTeamRunMock).toHaveBeenCalledWith({
        run_id: "run-team-1",
        revision: 2,
        tasks: [
          {
          ...
          },
          {
            task_id: "task-a",
            title: "Edited first",
            description: "Edited first details",
            owner_member_id: "teammate",
            sort_order: 1,
          },
        ],
    ...
    await waitFor(() =>
      expect(
        screen
          .getByRole("button", { name: "Confirm execution" })
          .getAttribute("disabled"),
      ).toBeNull(),
    );
    fireEvent.click(screen.getByRole("button", { name: "Confirm execution" }));
    await waitFor(() =>
      expect(confirmTeamRunMock).toHaveBeenCalledWith({
        run_id: "run-team-1",
        revision: 3,
      }),
    );
    expect(screen.getByTestId("team-plan-card").textContent).toContain(
      "executing",
    );
  ...
  it("projects confirmed tasks only to their owners and jumps from the Leader plan", async () => {
    const team = {
      ...
      members: [
        ...fixture.members,
        {
          ...
          agent_id: "agent-c",
          model: "model-c",
          execution_context_key: "ctx-teammate-2",
          created_at: "2026-08-31T00:00:00Z",
          updated_at: "2026-08-31T00:00:00Z",
        },
      ],
    ...
    const executing = teamRunForMembers(
      "executing",
      4,
      [
        teamTask(
    ...
    renderPage();
    await waitFor(() =>
      expect(screen.getByTestId("team-plan-card")).toBeTruthy(),
    );

    fireEvent.click(screen.getByTestId("team-member-teammate"));
    await waitFor(() =>
      expect(screen.getByTestId("team-task-card-task-a")).toBeTruthy(),
    );
    ...
    expect(screen.getByTestId("team-task-card-task-b").textContent).toContain(
      "Task B",
    );
  ...
  it("keeps restoration in the member Session path instead of exposing legacy run restore", async () => {
    getLatestTeamRunMock.mockResolvedValue(
      teamRun("executing", 4, [
        teamTask(
          "task-a",
          "Task A",
  ...
  it("renders Team facts before member history restoration resolves", async () => {
    let releaseHistory: ((tasks: TeamMemberTaskSnapshot[]) => void) | null =
      null;
    listTeamMemberTasksMock.mockImplementation(
      () =>
        new Promise((resolve) => {
    ...
    const resolveHistory = releaseHistory as
      ((tasks: TeamMemberTaskSnapshot[]) => void) | null;
    resolveHistory?.([]);
  });
  ...
  it("renders rich generic member events in place while inactive members keep running", async () => {
    const accepted = streamSnapshotWithItems(
      "teammate",
      "execution-direct",
      "Running",
      [
    ...
    fireEvent.click(screen.getByTestId("team-member-teammate"));
    fireEvent.change(screen.getByLabelText("Message content"), {
      target: { value: "Stream the result" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Send" }));
    await waitFor(() =>
      expect(screen.getAllByText("Stream the result")).toHaveLength(1),
    );
    await waitFor(() => expect(emitMemberSnapshot).not.toBeNull());

    await act(async () => {
      emitMemberSnapshot?.(
        streamSnapshotWithItems(
          "teammate",
          "execution-direct",
          "Running",
          [
    ...
    fireEvent.click(screen.getByTestId("team-member-leader"));
    expect(
      screen.getByLabelText("Message content").hasAttribute("disabled"),
    ).toBe(false);
    fireEvent.click(screen.getByTestId("team-member-teammate"));
    expect(screen.getByTestId("team-timeline").textContent).toContain(
      "Final delta",
...
function teamRunForMembers(
  ...
  return {
    run: {
      ...
      roster_snapshot: members.map(
        ({ id, role, sort_order, agent_id, model, execution_context_key }) => ({
          member_id: id,
          role,
          sort_order,
          agent_id,
