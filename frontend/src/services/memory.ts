...
export async function resolveMemoryContext(
  ...
  return memoryContextResultSchema.parse(
    await invoke("resolve_memory_context", {
      params: {
...
export async function getMemoryProject(
  projectPath: string,
): Promise<MemoryProjectView | null> {
  if (!isTauriRuntime()) return null;
  const value = await invoke("get_memory_project", {
...
export async function listMemoryPublicTasks(
  activeOnly = false,
): Promise<MemoryTaskView[]> {
  if (!isTauriRuntime()) return [];
  return memoryTaskViewSchema.array().parse(
    await invoke("list_memory_public_tasks", {
      params: { active_only: activeOnly },
    }),
  );
}

export async function getMemoryPublicTask(
  taskId: string,
): Promise<MemoryTaskView | null> {
  if (!isTauriRuntime()) return null;
  const value = await invoke("get_memory_public_task", {
    params: { task_id: taskId },
  });
  return value == null ? null : memoryTaskViewSchema.parse(value);
}

export async function cancelMemoryPublicTask(
  taskId: string,
): Promise<MemoryTaskView> {
  requireDesktop();
  return memoryTaskViewSchema.parse(
    await invoke("cancel_memory_public_task", {
      params: { task_id: taskId },
    }),
  );
}

export async function retryMemoryPublicTask(
  taskId: string,
): Promise<MemoryTaskView> {
  requireDesktop();
  return memoryTaskViewSchema.parse(
    await invoke("retry_memory_public_task", {
      params: { task_id: taskId },
    }),
  );
}
...
export async function searchMemoryRecall(params: {
  ...
  if (!isTauriRuntime()) return null;
  return memoryRecallSearchResultSchema.parse(
    await invoke("search_memory_recall", {
      params: { ...params, scope: params.scope ?? emptyMemoryScope() },
    }),
  );
}

export async function createMemoryRecallSession(
  scope: MemoryScope = emptyMemoryScope(),
): Promise<MemoryRecallSession> {
  requireDesktop();
  return memoryRecallSessionSchema.parse(
    await invoke("create_memory_recall_session", { params: { scope } }),
  );
}

export async function getMemoryRecallSession(
  sessionId: string,
): Promise<MemoryRecallSession> {
  requireDesktop();
  return memoryRecallSessionSchema.parse(
    await invoke("get_memory_recall_session", {
      params: { session_id: sessionId },
    }),
  );
}

export async function sendMemoryRecallTurn(
  sessionId: string,
  query: string,
): Promise<MemoryRecallSession> {
  requireDesktop();
  return memoryRecallSessionSchema.parse(
    await invoke("send_memory_recall_turn", {
      params: { session_id: sessionId, query },
    }),
  );
}

export async function cancelMemoryRecallTurn(
  turnId: string,
): Promise<MemoryRecallSession> {
  requireDesktop();
  return memoryRecallSessionSchema.parse(
    await invoke("cancel_memory_recall_turn", {
