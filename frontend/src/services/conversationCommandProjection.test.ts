...
describe("conversation command projection service", () => {
  ...
  it("uses the same raw-node contract in browser preview without copying the shell parser", async () => {
    ...
    await expect(
      projectConversationCommandParts({
        ...
        adapterVersion: "1.6.1",
        parts: [
          { partId: "preview-part", command: "printf divider && git status" },
        ],
      }),
    ).resolves.toEqual([
      {
        ...
        nodes: [
          {
            display_order: 0,
            command: "printf divider && git status",
            command_label: null,
          },
        ],
      },
  ...
  it("invalidates a cached projection when the source command label changes", async () => {
      ...
      .mockResolvedValueOnce([
        {
          ...
          projector_version: "shell-projector-v1",
          nodes: [
            { display_order: 0, command: "git status", command_label: "first" },
          ],
        },
      ])
      .mockResolvedValueOnce([
    ...
    await projectConversationCommandParts({
      ...
      adapterVersion: "1.6.1",
      parts: [
        { partId: "part-shell", command: "git status", commandLabel: "first" },
      ],
    });
