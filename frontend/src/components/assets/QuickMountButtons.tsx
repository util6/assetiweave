    ...
    <div className="flex w-fit max-w-full flex-wrap items-center justify-end gap-1">
      {visibleShortcuts.map((shortcut) => {
        const profile = profiles.find(
          (candidate) => candidate.id === shortcut.profileId,
        );
        const mountState = getMountDisplayState(
          statusByProfileId.get(shortcut.profileId),
        );
        ...
        const label =
          mountBlockedReason ??
          mountActionLabel(mountState, shortcut.profileName, t);
        ...
        const button = (
          <button
            className={clsx(
              ...
              mounted
                ? "shadow-glow ring-1 ring-theme-nav-active-border/30"
                : "opacity-60 hover:opacity-100",
              (mountState === "conflict" || mountState === "broken") &&
                "opacity-90",
              disabled &&
                "pointer-events-none cursor-not-allowed opacity-40 hover:opacity-40",
              !supported && "grayscale",
            )}
            aria-label={label}
            disabled={disabled}
            ...
            style={{
              ...
              backgroundColor: mounted
                ? `${shortcut.accentColor}24`
                : mountStateBackgroundColor(mountState, ringColor),
        ...
        return disabled ? (
          <span
            className="inline-grid cursor-not-allowed"
...
function mountStateBackgroundColor(
  state: MountDisplayState,
  ringColor: string,
) {
  if (state === "conflict" || state === "broken") {
