...
export function AssetMountCard({
  ...
  const displayIcon =
    shortcut?.displayIcon ?? profile.name.slice(0, 1).toUpperCase();
  ...
  const targetDir =
    mountStatus?.display_target_dir ??
    mountStatus?.target_dir ??
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        disabled &&
          "cursor-not-allowed opacity-55 hover:translate-y-0 hover:border-theme-control-border hover:bg-theme-control/80",
      )}
      disabled={disabled}
      ...
      <div className="flex min-w-0 items-start gap-3">
        <span
          ...
          style={{
            borderColor: mounted
              ? accentColor
              : mountCardRingColor(displayState, accentColor),
        ...
        >
          <MountCardStateRing
            color={mountCardRingColor(displayState, accentColor)}
            state={displayState}
          />
          <AppShortcutIcon
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-center justify-between gap-2">
            <p className="overflow-hidden text-ellipsis whitespace-nowrap text-body-sm font-bold text-on-surface">
              {profile.name}
            </p>
            <span
              className={clsx(
                ...
                mounted
                  ? "text-background"
                  : "border-theme-control-border bg-theme-control text-transparent",
              )}
              aria-hidden="true"
              style={
                mounted
            ...
            <span
              className={clsx(
                ...
                disabled || !supported
                  ...
                  : !mounted && mountCardStateTextClass(displayState),
              )}
              style={
                mounted && !disabled && supported
                  ? { color: accentColor }
                  : undefined
              }
            >
              <span
                className={clsx(
                  ...
                  disabled || !supported
                    ? "bg-status-conflict"
                    : !mounted && mountCardStateDotClass(displayState),
                )}
                aria-hidden="true"
                style={
                  mounted && !disabled && supported
              ...
              {disabled
                ? t("mount.blocked")
                : supported
                  ? t(`mount.display.${displayState}` as TranslationKey)
                  : t("mount.unsupported")}
            </span>
          </div>
        </div>
...
function mountCardStateClass(state: MountDisplayState) {
  if (state === "conflict" || state === "broken")
    return "border-status-remove/50 bg-status-remove/10 hover:border-status-remove/65";
...
function mountCardRingColor(state: MountDisplayState, accentColor: string) {
  if (state === "conflict" || state === "broken")
    return "rgb(var(--color-status-remove))";
  return accentColor;
}
