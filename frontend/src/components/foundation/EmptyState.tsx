...
const EmptyState = React.forwardRef<HTMLDivElement, EmptyStateProps>(
  ({ actions, className, description, icon, title, ...props }, ref) => (
    ...
    >
      {icon && (
        <div className="mb-3 grid size-11 place-items-center rounded-2xl border border-theme-control-border/70 bg-theme-control/70 text-primary shadow-[0_8px_20px_rgb(var(--theme-glow)/0.12),inset_0_1px_0_rgb(var(--theme-inset-highlight)/0.2)]">
          {icon}
        </div>
      )}
