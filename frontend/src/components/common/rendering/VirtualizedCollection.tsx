...
function validateKeys<Item>(
  items: readonly Item[],
  getItemKey: VirtualizedCollectionProps<Item>["getItemKey"],
): string[] {
  ...
  keys.forEach((key) => {
    if (typeof key !== "string" || key.trim().length === 0) {
      throw new Error(
        "VirtualizedCollection requires non-empty string item keys",
      );
    }
    if (seen.has(key))
      throw new Error(`VirtualizedCollection duplicate item key: ${key}`);
    seen.add(key);
  });
  return keys;
}
...
function VirtualizedCollectionInner<Item>(
  ...
  const limitedPinnedKeys = useMemo(
    () => limitKeys(pinnedKeys, 4, "pinned"),
    [pinnedKeys],
  );
  const limitedEagerKeys = useMemo(
    () => limitKeys(eagerKeys, 2, "eager"),
    [eagerKeys],
  );
  ...
  const renderBoundary = useCallback(
    (item: Item, index: number, key: string, priority?: RenderPriority) => (
  ...
  return (
    ...
    >
      {virtualItems.map((virtualItem) => {
        ...
        const priority = priorityForVirtualItem(
          virtualItem,
...
export const VirtualizedCollection = forwardRef(VirtualizedCollectionInner) as <
  Item,
>(
  props: VirtualizedCollectionProps<Item> & {
    ref?: React.ForwardedRef<VirtualizedCollectionHandle>;
  },
) => React.ReactElement;
