...
let intersectionInstances: Array<{
  disconnect: () => void;
...
describe("RenderVisibilityRegistry", () => {
  it("uses one shared observer and prioritizes visible and directional overscan items", () => {
    ...
    registry.register({
      element: visible,
      key: "visible",
      onPriorityChange: (value) => priorities.push(value),
    });
    registry.register({
      element: ahead,
      key: "ahead",
    ...
    intersectionCallbacks[0]?.([
      ...
      {
        boundingClientRect: { top: 450, bottom: 500 } as DOMRect,
        isIntersecting: true,
        target: ahead,
      },
      {
        boundingClientRect: { top: -100, bottom: -20 } as DOMRect,
        isIntersecting: true,
        target: behind,
      },
    ]);
