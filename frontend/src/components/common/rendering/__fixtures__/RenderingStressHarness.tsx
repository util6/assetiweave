...
export interface RenderingStressHarnessProps {
  render: (
    question: ReturnType<typeof createConversationRenderingStressFixture>,
  ) => ReactNode;
}
...
export function RenderingStressHarness({
  render,
}: RenderingStressHarnessProps): React.ReactElement | null {
  if (
    !renderingStressHarnessEnvironment?.DEV &&
    renderingStressHarnessEnvironment?.MODE !== "test"
  )
    return null;
