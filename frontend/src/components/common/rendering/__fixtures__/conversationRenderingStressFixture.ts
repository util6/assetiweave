    ...
    size === 0
      ...
      : size === 1
        ...
        : Array.from(
            { length: 12 },
            (_, paragraph) =>
              `Long rendering prompt ${number}, paragraph ${paragraph + 1}, exercises an unusually large Turn.`,
          ).join("\n\n");
...
function createMarkdownAnswer(index: number) {
  const paragraphs = Array.from(
    { length: 8 },
    (_, paragraph) =>
      `Turn ${index + 1} markdown paragraph ${paragraph + 1} is deliberately deterministic for scroll rendering.`,
  );
