...
export function SourceBulkMountControls({
  ...
  const availableShortcuts = appShortcuts
    ...
    .filter((shortcut) =>
      profiles.some(
        (profile) => profile.id === shortcut.profileId && profile.enabled,
      ),
    );

  if (
    variant === "inline" &&
    (skillAssets.length === 0 || availableShortcuts.length === 0)
  ) {
    return null;
  }
  ...
  if (
    variant === "panel" &&
    (skillAssets.length === 0 || availableShortcuts.length === 0 || blocked)
  ) {
    return (
  ...
  return (
    ...
    >
      ...
      <div
        className={clsx(
          variant === "panel"
            ? "grid grid-cols-2 gap-2 max-[1180px]:grid-cols-1"
      ...
      >
        {availableShortcuts.map((shortcut) => {
          const mountedCount = skillAssets.filter((asset) => {
            ...
            ).find((status) => status.profile_id === shortcut.profileId);
            return mountStatus?.state === "mounted";
          }).length;
          const allMounted =
            skillAssets.length > 0 && mountedCount === skillAssets.length;
          const hasPartialMounts = mountedCount > 0 && !allMounted;
          const label = t(
            allMounted ? "source.bulk.unselectAll" : "source.bulk.selectAll",
            {
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                allMounted &&
                  "shadow-[inset_0_1px_0_rgb(var(--theme-inset-highlight)/0.28)]",
              )}
            ...
            >
              ...
              <span className="min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-body-sm font-semibold">
                {variant === "panel"
                  ...
                  : t("source.bulk.progress", {
                      selected: mountedCount,
...
function bulkMountButtonStyle(
  accentColor: string,
  allMounted: boolean,
  hasPartialMounts: boolean,
) {
  return {
    backgroundColor: allMounted
      ? `${accentColor}24`
      : hasPartialMounts
        ? `${accentColor}18`
        : `${accentColor}10`,
    borderColor: allMounted
      ? accentColor
      : hasPartialMounts
        ? `${accentColor}99`
        : `${accentColor}66`,
