    ...
    <div className="aurora-detail-surface overflow-hidden">
      <div className="aurora-workbench-header flex items-center justify-between px-4 py-3">
        ...
        <span className="font-mono text-body-sm text-primary">
          {t("plan.actions", { count: plan.actions.length })}
        </span>
      </div>
        ...
        {plan.actions.slice(0, 16).map((action) => (
          ...
          >
            <span className={planActionClass(action.action_type)}>
              {deploymentActionLabel(action.action_type, t)}
            </span>
            ...
            <div className="min-w-0">
              <p className="overflow-hidden text-ellipsis whitespace-nowrap font-mono text-body-sm text-on-surface">
                {action.display_target_path ??
                  abbreviateHomePath(action.target_path)}
              </p>
              <p className="mt-1 overflow-hidden text-ellipsis whitespace-nowrap text-body-sm text-outline">
                {translatePlanReason(action.reason, t)}
              </p>
            </div>
          </div>
        ))}
