const zh = {
  ...
  "conversation.sync.partialFailureAdvice": "同步任务已完成，但部分来源失败。已成功导入可用来源；请检查失败来源、重新信任变更过的 adapter 或修复来源配置后，再单独同步失败来源。",
  ...
  "conversation.sync.web.partialFailureAdvice": "同步任务已完成，但部分网页来源失败。已成功导入可用来源；请刷新浏览器登录或网页 token、重新信任变更过的 adapter，然后单独同步失败来源。",
...
const en: Record<TranslationKey, string> = {
  ...
  "conversation.sync.partialFailureAdvice": "The sync task completed, but some sources failed. Usable sources were imported; review the failed source, re-trust changed adapters or fix source settings, then sync the failed source again.",
  ...
  "conversation.sync.web.partialFailureAdvice": "Some web sources failed even though the sync task completed. Successful sources were imported; refresh browser login or re-trust changed adapters, then sync the failed source again.",
